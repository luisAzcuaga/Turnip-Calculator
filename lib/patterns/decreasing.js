import { RATES, THRESHOLDS } from "../constants.js";
...
// Uses constants from constants.js (RATES)
//
// Game algorithm:
...
export function calculateDecreasingPattern(periodIndex, base, knownPrices = []) {
  ...
  if (knownPrices.length >= 1) {
    ...
    if (periodsAhead > 0) {
      ...
      const minRate = Math.max(RATES.FLOOR, lastRate - (RATES.DECREASING.MAX_DROP_RATE * periodsAhead));
      const maxRate = Math.max(RATES.FLOOR, lastRate - (RATES.DECREASING.MIN_DROP_RATE * periodsAhead));
      return {
  ...
  const minRate = Math.max(RATES.FLOOR, (RATES.DECREASING.START_MIN - (periodIndex * RATES.DECREASING.MAX_DROP_RATE)));
  const maxRate = Math.max(RATES.FLOOR, (RATES.DECREASING.START_MAX - (periodIndex * RATES.DECREASING.MIN_DROP_RATE)));
