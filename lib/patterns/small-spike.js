...
export function scoreSmallSpike(knownPrices, buyPrice) {
  ...
  if (ratio >= RATES.SMALL_SPIKE.PEAK_RATE_MIN && ratio < RATES.SMALL_SPIKE.PEAK_RATE_MAX) {
    ...
    if (ratio >= THRESHOLDS.SMALL_SPIKE_PERFECT_MIN && ratio <= THRESHOLDS.SMALL_SPIKE_PERFECT_MAX) {
      score += 90;
      reasons.push(`✅ ¡Pico perfecto! ${maxPrice} bayas (${Math.round(ratio * 100)}%) en rango ideal de Small Spike (140-200%)`);
    } else {
      score += 70;
      reasons.push(`✅ Pico detectado ${maxPrice} bayas (${Math.round(ratio * 100)}%) en rango de Small Spike (140-200%)`);
    }
  } else if (ratio >= THRESHOLDS.SMALL_SPIKE_PRE_PEAK && ratio < RATES.SMALL_SPIKE.PEAK_RATE_MIN) {
    score += 40;
    reasons.push(`⚠️ Precio ${maxPrice} bayas (${Math.round(ratio * 100)}%) podría ser pre-pico de Small Spike`);
    ...
    if (maxPriceIndex !== -1 && maxPriceIndex < knownPrices.length - 1) {
      ...
      if (hasSharpDrop) {
        ...
        score = 0;
        reasons.push(`❌ El precio máximo ${maxPrice} bayas no alcanzó 140%. Los precios subieron y bajaron sin formar un pico válido.`);
      }
    }
  } else if (ratio >= RATES.SMALL_SPIKE.PEAK_RATE_MAX) {
    smallSpikeRejected = true;
    score = 0;
    reasons.push(`❌ Precio ${maxPrice} bayas (${Math.round(ratio * 100)}%) excede 200% (esto es Large Spike)`);
  } else {
    ...
    if (maxPriceIndex !== -1 && maxPriceIndex < knownPrices.length - 1) {
      ...
      if (hasSharpDrop) {
        smallSpikeRejected = true;
        score = 0;
        reasons.push(`❌ El precio máximo ${maxPrice} bayas (${Math.round(ratio * 100)}%) no alcanzó 140%. Los precios subieron y bajaron sin formar un pico válido.`);
      } else {
        reasons.push(`⏳ Esperando confirmación. Máximo ${maxPrice} bayas (${Math.round(ratio * 100)}%) es válido para fase inicial. Si el siguiente período se mantiene en 90-140%, sugiere Pico Pequeño`);
      }
    } else {
      reasons.push(`⏳ Esperando confirmación. Máximo ${maxPrice} bayas (${Math.round(ratio * 100)}%) es válido para fase inicial. Si el siguiente período se mantiene en 90-140%, sugiere Pico Pequeño`);
    }
  ...
  if (!smallSpikeRejected) {
    ...
    if (lsSequence.detected && !lsSequence.hasDataAfterSequence) {
      ...
      if (ratio >= RATES.SMALL_SPIKE.PEAK_RATE_MIN && ratio < RATES.SMALL_SPIKE.PEAK_RATE_MAX) {
        ...
        reasons.push(`ℹ️ La secuencia ${Math.round(spikePhase1.rate * 100)}% → ${Math.round(spikePhase2.rate * 100)}% también podría ser Large Spike, pero el pico de ${Math.round(ratio * 100)}% es consistente con Small Spike.`);
      } else {
        score -= 50;
        reasons.push(`⚠️ La secuencia ${Math.round(spikePhase1.rate * 100)}% → ${Math.round(spikePhase2.rate * 100)}% también coincide con Large Spike. El pico real podría ser mayor (200-600%).`);
      }
    }
