...
export function reasonsToRejectSmallSpike(knownPrices, buyPrice) {
  ...
  // 1. SINGLE-PRICE: Monday AM must be in pre-spike range (40-90%) OR spike phase 0 range (90-140%)
  // if the spike can start at Monday AM (SMALL_SPIKE_START_MIN === 0).
  ...
  if (mondayAM) {
    ...
    if (mondayRatio > RATES.SMALL_SPIKE.START_MAX) {
      const phase0 = RATES.SMALL_SPIKE.SPIKE_PHASES[0];
      const couldBeSpikePhase0 = PERIODS.SMALL_SPIKE_START_MIN === PERIODS.MONDAY_AM &&
        mondayRatio >= phase0.min && mondayRatio <= phase0.max;
      if (!couldBeSpikePhase0) {
  ...
  // 4. SLOPE
  const dropRateCheck = validatePreSpikeDropRate(knownPrices, false, buyPrice);
  ...
  // 5. PATTERN ANALYSIS: Multiple rise-fall cycles → Fluctuating, not Spike
  if (knownPrices.length >= 4) {
    for (let i = 1; i < knownPrices.length - 1; i++) {
      ...
      if (curr.price > prev.price && curr.price > next.price) {
        const localMaxRatio = priceRatio(curr.price, buyPrice);
...
export function scoreSmallSpike(knownPrices, buyPrice) {
  ...
  if (confirmation.detected && confirmation.isLargeSpike === null) {
    ...
    const phase2Pct = Math.round(parseFloat(confirmation.percent));
    const peakRange = `${RATES.LARGE_SPIKE.SPIKE_PHASES[2].min * 100}%-${RATES.LARGE_SPIKE.SPIKE_PHASES[2].max * 100}%`;
    if (ratio >= RATES.SMALL_SPIKE.PEAK_RATE_MIN && ratio <= RATES.SMALL_SPIKE.PEAK_RATE_MAX) {
      score -= 15;
      reasons.push(`
        ⚠️ La secuencia ${phase1Pct}${phase2Pct}% también podría ser Pico Grande, pero el pico de ${Math.round(ratio * 100)}% es consistente con Pico Pequeño.
      `);
    } else {
      score -= 50;
      reasons.push(`
        ⚠️ La secuencia ${phase1Pct}${phase2Pct}% también coincide con Pico Grande.
