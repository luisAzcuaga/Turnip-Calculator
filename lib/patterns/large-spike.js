...
export function reasonsToRejectLargeSpike(knownPrices, buyPrice) {
  ...
  if (maxKnownIndex >= PERIODS.LAST_PERIOD && maxRatio < RATES.SMALL_SPIKE.PEAK_RATE_MIN) {
    ...
    rejectReasons.push(`
      Es ${lastPeriodName} y el precio máximo fue $${maxPrice} (${Math.round(maxRatio * 100)}%). Pico grande necesita un pico de ${thirdPhaseRange}.
    El pico puede empezar entre ${spikeRange.minName} y ${spikeRange.maxName} (períodos ${spikeRange.min}-${spikeRange.max}).
    `);
    return rejectReasons;
  }
    ...
    if (maxPriceIndex !== -1 && maxPriceIndex < knownPrices.length - 1) {
      ...
      if (hasSharpDrop) {
        rejectReasons.push(`
          ...
          El pico ya pasó y fue muy bajo para Large Spike.`);
        return rejectReasons;
      }
    }
  }
  ...
  if (confirmation.detected && confirmation.isLargeSpike === false) {
    ...
    rejectReasons.push(
      ...
      Large Spike necesita ${secondPhaseRange} en el Período 2 seguido de ${thirdPhaseRange} en Período 3.`);
    return rejectReasons;
  }
  ...
  if (maxRatio >= RATES.SMALL_SPIKE.PEAK_RATE_MIN && maxRatio < RATES.SMALL_SPIKE.PEAK_RATE_MAX && maxKnownIndex >= PERIODS.LATE_WEEK_START) {
    if (!confirmation.detected || !confirmation.isLargeSpike) {
      ...
      if (!hasRapidIncrease) {
        ...
        Ya es tarde en la semana sin señales de Large Spike. Large Spike puede empezar entre ${spikeRange.minName} y ${spikeRange.maxName} (períodos ${spikeRange.min}-${spikeRange.max}).`);
        return rejectReasons;
      }
    }
  }
...
export function scoreLargeSpike(knownPrices, buyPrice) {
  ...
  if (maxPriceRatio >= RATES.LARGE_SPIKE.PEAK_RATE_MIN) {
    score += 100;
    reasons.push(`
      ✅ ¡Pico enorme detectado! $${maxPrice} corresponde al ${Math.round(maxPriceRatio * 100)}% del precio base. Esto confirma Large Spike
      `);
    ...
    if (maxPriceRatio < THRESHOLDS.LARGE_SPIKE_NEAR_LIMIT) {
      ...
      score += 30;
      reasons.push(`
        ⚠️ El precio máximo hasta ahora es de $${maxPrice} (${Math.round(maxPriceRatio * 100)}%) cerca del límite de Large Spike (190-200%)
        `);
    }
  } else {
    score += 5;
  ...
  if (lsSequence.detected && !lsSequence.hasDataAfterSequence) {
    ...
    if (spikePhase2.rate >= RATES.SMALL_SPIKE.PEAK_RATE_MIN && spikePhase2.rate < RATES.SMALL_SPIKE.PEAK_RATE_MAX) {
      ...
      score += 80;
      const largeSpikeSequence = `$${spikePhase1.price} (${Math.round(spikePhase1.rate * 100)}%) → $${spikePhase2.price} (${Math.round(spikePhase2.rate * 100)}%)`;
      reasons.push(
        `✅ Secuencia Pico Grande: ${largeSpikeSequence}.
        El pico real (${thirdPhaseRange}) puede venir en el siguiente período.
        `);
    }
  }
