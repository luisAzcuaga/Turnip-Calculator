...
export function reasonsToRejectLargeSpike(knownPrices, buyPrice) {
  ...
  // 4. SLOPE
  const dropRateCheck = validatePreSpikeDropRate(knownPrices, true, buyPrice);
  if (dropRateCheck.invalid) {
    rejectReasons.push(dropRateCheck.reason);
    return rejectReasons;
  }
...
export function scoreLargeSpike(knownPrices, buyPrice) {
  ...
  // Bonus if we detect an ambiguous P1→P2 sequence with no prices after
  // Uses detectSpikeConfirmation (same as rejection) to stay in sync with rejection logic
  const thirdPhaseRange = `${RATES.LARGE_SPIKE.SPIKE_PHASES[2].min * 100}%-${RATES.LARGE_SPIKE.SPIKE_PHASES[2].max * 100}%`;
  const confirmation = detectSpikeConfirmation(knownPrices, buyPrice);
  if (confirmation.detected && confirmation.isLargeSpike === null) {
    const phase1Pct = confirmation.spikePhase1 ? `${Math.round(confirmation.spikePhase1.rate * 100)}% → ` : '';

    score += 30;
    reasons.push(`
        ⚠️ Secuencia ${phase1Pct}${Math.round(parseFloat(confirmation.percent))}% es ambigua. Podría ser Pico Grande (esperando ${thirdPhaseRange}).
        `);
  }

  // Bonus for ascending momentum: consecutive rises toward a potential peak
  // When the max price is still climbing (latest known) and in the spike range,
  // with 2+ consecutive rises, the spike hasn't peaked yet — P3 (200-600%) may follow
  const maxIsLatest = knownPrices[knownPrices.length - 1].price === maxPrice;
  if (maxIsLatest && maxPriceRatio >= RATES.SMALL_SPIKE.PEAK_RATE_MIN) {
    let consecutiveRises = 0;
    for (let i = knownPrices.length - 1; i > 0; i--) {
      if (knownPrices[i].price > knownPrices[i - 1].price) consecutiveRises++;
      else break;
    }
    if (consecutiveRises >= 2) {
      score += 50;
      reasons.push(`✅ ${consecutiveRises} subidas consecutivas hasta ${Math.round(maxPriceRatio * 100)}% — el pico aún no llegó, podría alcanzar ${thirdPhaseRange}`);
    }
  }
