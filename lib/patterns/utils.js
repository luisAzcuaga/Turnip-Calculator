import { DAYS_CONFIG, PERIODS, RATES, THRESHOLDS } from "../constants.js";

// Shared utilities for price patterns
// Helper functions used by multiple patterns
// Uses constants from constants.js (THRESHOLDS, PERIODS, RATES, etc.)
 ...
 * @param {number} basePrice - Base buy price
 * @param {number} rate - Rate multiplier (e.g. 0.85 for 85%)
 * @returns {number} - Floor-rounded price
 */
 ...
 * @param {number} basePrice - Base buy price
 * @param {number} rate - Rate multiplier (e.g. 1.40 for 140%)
 * @returns {number} - Ceil-rounded price
 */
 ...
 * @param {number} price - Observed price in bells
 * @param {number} basePrice - Base buy price
 * @returns {number} - Ratio (e.g. 1.40 means 140% of base)
 */
...
export function isValidRateDrop(previousPrice, currentPrice, buyPrice) {
  ...
  return {
    valid: rateDrop <= RATES.DECREASING.MAX_DROP_RATE,
    rateDrop: rateDrop
  };
}
 ...
 * @returns {number} - Maximum valid price for the Decreasing pattern at that period
 */
export function decreasingMaxForPeriod(basePrice, periodIndex) {
  const rate = RATES.DECREASING.START_MAX - (periodIndex * RATES.DECREASING.MIN_DROP_RATE);
 ...
 * @param {number} basePrice - Base buy price
 * @returns {number} - Absolute floor price (RATES.FLOOR × basePrice)
 */
...
/**
 * Calculates the valid Monday AM price range for the Large Spike pattern (85-90%)
 * @param {number} basePrice - Base buy price
 * @returns {{min: number, max: number}} - Valid price range for Monday AM
 */
 ...
 * @param {number} periodIndex - Period index (0-11)
 * @returns {string} - Period name (e.g. "Lunes AM")
 */
 ...
 * @param {boolean} isLargeSpike - True for Large Spike, false for Small Spike
 * @returns {{min: number, max: number, minName: string, maxName: string}} - Start period range with human-readable names
 */
export function getSpikeStartRange(isLargeSpike) {
  return {
    ...
    minName: getPeriodName(isLargeSpike ? PERIODS.LARGE_SPIKE_START_MIN : PERIODS.SMALL_SPIKE_START_MIN),
    maxName: getPeriodName(PERIODS.SPIKE_START_MAX),
  };
}
 ...
 * @returns {number} - Average rate drop per period (e.g. 0.04 for 4%)
 */
 ...
 * @param {Array<{index: number, price: number}|number>} prices - Known prices (objects or plain numbers)
 * @param {number} buyPrice - Base buy price
 * @returns {{detected: boolean, startIndex: number}} - Whether a spike start was detected and at which array index
 */
 ...
 * @param {Array<{index: number, price: number}>} knownPrices - Known prices sorted by index
 * @param {number} minSpikeStart - Minimum period index where the spike can start
 * @param {number} maxSpikeStart - Maximum period index where the spike can start
 * @param {boolean} isLargeSpike - True if large spike, false if small
 * @param {number} buyPrice - Base buy price
 * @returns {number} - Estimated period index where the spike starts
 */
export function detectSpikePhaseStart(knownPrices, minSpikeStart, maxSpikeStart, isLargeSpike, buyPrice) {
  ...
  if (maxPriceData && hasDecliningPricesAfterMax) {
    ...
    if (isLargeSpike && ratio >= RATES.LARGE_SPIKE.PEAK_RATE_MIN) {
      const estimatedSpikeStart = Math.max(minSpikeStart, maxPriceData.index - 2); // Peak is SPIKE_PHASES[2], so start = peak - 2
    ...
    if (!isLargeSpike && ratio >= RATES.SMALL_SPIKE.PEAK_RATE_MIN) {
      const estimatedSpikeStart = Math.max(minSpikeStart, maxPriceData.index - 3); // Peak is SPIKE_PHASES[3], so start = peak - 3
      return Math.min(maxSpikeStart, estimatedSpikeStart);
    }
  }
  ...
  // PRIORITY 3: Max is the latest known price and is in the spike peak range.
  // We're in the spike right now — infer the start from the peak position.
  // This handles cases where pre-spike prices are missing (no trend reversal detected).
  const maxIsLatestKnown = maxPriceIndex === Math.max(...knownPrices.map(p => p.index));
  if (maxPriceData && maxIsLatestKnown) {
    if (isLargeSpike && ratio >= RATES.LARGE_SPIKE.PEAK_RATE_MIN) {
 ...
 * @param {Array<{index: number, price: number}>} knownPrices - Known prices sorted by index
 * @param {number} buyPrice - Base buy price
 * @returns {{detected: false}|{detected: true, spikePhase1: {price: number, rate: number, index: number}, spikePhase2: {price: number, rate: number, index: number}, hasDataAfterSequence: boolean}}
 */
...
/**
 * Validates the drop rate in the pre-spike phase for both spike types.
 * @param {Array<{index: number, price: number}>} knownPrices - Known prices sorted by index
 * @param {boolean} isLargeSpike - True for Large Spike, false for Small Spike
 * @param {number} buyPrice - Base buy price
 * @returns {{invalid: false}|{invalid: true, reason: string}}
 */
export function validatePreSpikeDropRate(knownPrices, isLargeSpike, buyPrice) {
 ...
 * @param {Array<{index: number, price: number}>} knownPrices - Known prices sorted by index
 * @param {string} spikeType - Pattern name for error messages (e.g. "Large Spike")
 * @returns {{tooLate: false}|{tooLate: true, reason: string}}
 */
 ...
 * @param {Array<{index: number, price: number}>} knownPrices - Known prices sorted by index
 * @param {number} buyPrice - Base buy price
 * @returns {{detected: false}|{detected: true, isLargeSpike: boolean|null, spikePhase1?: {price: number, rate: number, index: number}, price: number, percent: string, day: string}}
 *   - `isLargeSpike: true`  — confirmed Large Spike (max price ≥ 200%)
 *   - `isLargeSpike: null`  — ambiguous; confirmation in 140-200%, could be either spike type
 *   - `isLargeSpike: false` — confirmed NOT Large Spike (confirmation < 140%, or data after sequence with max < 200%)
 *   - `spikePhase1` is only present when detected via the P1→P2 sequence path
 */
export function detectSpikeConfirmation(knownPrices, buyPrice) {
  ...
  if (sequence.detected) {
    const { spikePhase1, spikePhase2, hasDataAfterSequence } = sequence;
    ...
    return { detected: true, isLargeSpike, spikePhase1, price: spikePhase2.price, percent, day: getPeriodName(spikePhase2.index) };
  }
  ...
  let isLargeSpike;
  if (confirmationRate >= RATES.LARGE_SPIKE.PEAK_RATE_MIN) {
    isLargeSpike = true;  // ≥ 200%: confirmed Large Spike
  } else if (confirmationRate >= RATES.SMALL_SPIKE.PEAK_RATE_MIN) {
    isLargeSpike = null;  // 140-200%: ambiguous, could be either spike type
  } else {
    isLargeSpike = false; // < 140%: below spike peak range
  }
  return {
    detected: true,
    isLargeSpike,
 ...
 * @param {number} base - Base buy price
 * @param {Array<{index: number, price: number}>} phaseKnownPrices - Known prices already filtered to this phase
 * @param {number} periodsIntoPhase - Periods elapsed since phase start (for no-data branch)
 * @param {number} startMinRate - Minimum rate at phase start (e.g. 0.85 for 85%)
 * @param {number} startMaxRate - Maximum rate at phase start (e.g. 0.90 for 90%)
 * @param {boolean} decayMinRate - If true, minRate decays per period; if false, it is fixed at startMinRate
 * @returns {{min: number, max: number}} - Predicted price range
 */
export function calculateDecreasingPhaseRange(
  ...
  if (phaseKnownPrices.length >= 1) {
    ...
    if (periodsAhead > 0) {
      ...
      const minProjectedRate = lastKnownRate - (RATES.DECREASING.MAX_DROP_RATE * periodsAhead);
      const maxProjectedRate = lastKnownRate - (RATES.DECREASING.MIN_DROP_RATE * periodsAhead);
      const minProjected = base * Math.max(RATES.FLOOR, minProjectedRate);
      const maxProjected = base * Math.max(RATES.FLOOR, maxProjectedRate);
      return {
  ...
  const minRate = decayMinRate
    ? Math.max(RATES.FLOOR, startMinRate - (periodsIntoPhase * RATES.DECREASING.MAX_DROP_RATE))
    : startMinRate;
  const maxRate = Math.max(RATES.FLOOR, startMaxRate - (periodsIntoPhase * RATES.DECREASING.MIN_DROP_RATE));
  return {
