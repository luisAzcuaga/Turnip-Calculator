...
import { displayPriceChart } from "./chart.js";
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  const resultsGrid = document.querySelector('.pattern-panels');

  const VIEW_PREF_KEY = 'turnip_view_pref';
  ...
  // View toggle: compact (probability panel) <-> detailed (explanations)
  ...
  function calculatePrediction() {
    ...
    const results = predictor.execute();
  ...
  function applyViewPreference() {
    const view = localStorage.getItem(VIEW_PREF_KEY) ?? 'compact';
    const isCompact = view === 'compact';
    resultsGrid.classList.toggle('compact-view', isCompact);
    document.getElementById('toggleViewBtn').textContent = isCompact
      ? 'Ver análisis detallado →'
      : '← Ver resumen';
  }

  function toggleView() {
    const isCompact = resultsGrid.classList.contains('compact-view');
    const newView = isCompact ? 'detailed' : 'compact';
    resultsGrid.classList.toggle('compact-view', !isCompact);
    document.getElementById('toggleViewBtn').textContent = isCompact
      ? '← Ver resumen'
      : 'Ver análisis detallado →';
  ...
  function displayResults(results) {
    ...
    // Show pattern probability summary
    document.getElementById('patternSummary').innerHTML = buildProbabilityPanelHTML(results.patternName, results.allProbabilities);

    // Fill inputs with predictions
    fillInputsWithPredictions(results.predictions);

    // Show price trend chart
    displayPriceChart(results.predictions, parseInt(buyPriceInput.value));
    ...
    document.getElementById('patternDetail').innerHTML = buildRejectionReasonsHTML(
