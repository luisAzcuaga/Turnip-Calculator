import { DAYS_CONFIG } from "../constants.js";

let priceChartInstance = null;

function buildChartData(predictions, buyPrice, labelCount) {
  const rangeMax = [];
  const rangeMin = [];
  const confirmedLine = [];

  DAYS_CONFIG.forEach((day, index) => {
    const prediction = predictions[day.key];
    const next = DAYS_CONFIG[index + 1] ? predictions[DAYS_CONFIG[index + 1].key] : null;
    const isLastConfirmed = prediction.isUserInput && (!next || !next.isUserInput);

    if (prediction.isUserInput) {
      // Only anchor the band at the transition point to avoid tension artifacts
      rangeMax.push(isLastConfirmed ? prediction.min : null);
      rangeMin.push(isLastConfirmed ? prediction.min : null);
      confirmedLine.push(prediction.min);
    } else {
      rangeMax.push(prediction.max);
      rangeMin.push(prediction.min);
      confirmedLine.push(null);
    }
  });

  return {
    rangeMax,
    rangeMin,
    buyPriceLine: Array(labelCount).fill(buyPrice),
    confirmedLine
  };
}

// Los colores de la gráfica viven en los tokens --chart-* de :root (styles.css)
// y se leen en cada render; al reutilizar la instancia de Chart.js,
// applyChartColors los vuelve a asignar.
function readChartColors() {
  const styles = getComputedStyle(document.documentElement);
  const token = (name) => styles.getPropertyValue(name).trim();

  return {
    rangeLine: token('--chart-range-line'),
    rangeFill: token('--chart-range-fill'),
    buyLine: token('--chart-buy-line'),
    confirmedLine: token('--chart-confirmed-line'),
    pointBorder: token('--chart-point-border'),
    grid: token('--chart-grid'),
    axisText: token('--chart-axis-text')
  };
}

function applyChartColors(chart, colors) {
  const [rangeMaxSet, rangeMinSet, buyPriceSet, confirmedSet] = chart.data.datasets;

  rangeMaxSet.borderColor = colors.rangeLine;
  rangeMaxSet.backgroundColor = colors.rangeFill;
  rangeMinSet.borderColor = colors.rangeLine;
  buyPriceSet.borderColor = colors.buyLine;
  confirmedSet.borderColor = colors.confirmedLine;
  confirmedSet.pointBackgroundColor = colors.confirmedLine;
  confirmedSet.pointBorderColor = colors.pointBorder;

  const { x, y } = chart.options.scales;
  y.grid.color = colors.grid;
  y.ticks.color = colors.axisText;
  y.title.color = colors.axisText;
  x.ticks.color = colors.axisText;
}

// La CSS del tema se actualiza sola, pero la instancia de Chart.js está
// cacheada y hay que repintarla. Lo dispara lib/ui/theme.js, que es el único
// que decide el tema (preferencia guardada, o la del sistema si no hay).
export function refreshChartTheme() {
  if (!priceChartInstance) return;
  applyChartColors(priceChartInstance, readChartColors());
  priceChartInstance.update();
}

export function displayPriceChart(predictions, buyPrice) {
  const canvas = document.getElementById('priceChart');
  if (!canvas) return;

  const labels = DAYS_CONFIG.map(d => d.name);
  const { rangeMax, rangeMin, buyPriceLine, confirmedLine } = buildChartData(predictions, buyPrice, labels.length);

  if (priceChartInstance) {
    const datasets = priceChartInstance.data.datasets;
    datasets[0].data = rangeMax;
    datasets[1].data = rangeMin;
    datasets[2].data = buyPriceLine;
    datasets[3].data = confirmedLine;
    datasets[3].pointRadius = confirmedLine.map(v => v !== null ? 5 : 0);
    applyChartColors(priceChartInstance, readChartColors());
    priceChartInstance.update();
    return;
  }

  const colors = readChartColors();
  const ctx = canvas.getContext('2d');
  priceChartInstance = new Chart(ctx, { // eslint-disable-line no-undef
    type: 'line',
    data: {
      labels,
      datasets: [
        // Upper band boundary — fills down to lower band
        {
          label: 'Rango estimado',
          data: rangeMax,
          borderColor: colors.rangeLine,
          borderWidth: 1,
          pointRadius: 0,
          fill: '+1',
          backgroundColor: colors.rangeFill,
          tension: 0.3
        },
        // Lower band boundary
        {
          label: 'Mínimo',
          data: rangeMin,
          borderColor: colors.rangeLine,
          borderWidth: 1,
          pointRadius: 0,
          fill: false,
          tension: 0.3
        },
        // Buy price reference line
        {
          label: 'Precio de compra',
          data: buyPriceLine,
          borderColor: colors.buyLine,
          borderWidth: 1,
          borderDash: [6, 4],
          pointRadius: 0,
          fill: false
        },
        // Confirmed prices — solid line with dots
        {
          label: 'Confirmado',
          data: confirmedLine,
          borderColor: colors.confirmedLine,
          borderWidth: 2.5,
          pointRadius: confirmedLine.map(v => v !== null ? 5 : 0),
          pointBackgroundColor: colors.confirmedLine,
          pointBorderColor: colors.pointBorder,
          pointBorderWidth: 2,
          fill: false,
          spanGaps: false,
          tension: 0.3
        }
      ]
    },
    options: {
      responsive: true,
      maintainAspectRatio: false,
      interaction: {
        mode: 'index',
        intersect: false
      },
      plugins: {
        legend: {
          display: false,
        },
        tooltip: false
      },
      scales: {
        y: {
          suggestedMin: 0,
          title: {
            display: true,
            text: 'Precio (bayas)',
            font: { size: 14, weight: 'bold' },
            color: colors.axisText
          },
          ticks: {
            color: colors.axisText
          },
          grid: {
            color: colors.grid
          }
        },
        x: {
          ticks: {
            callback: function(_val, index) {
              return window.innerWidth < 500 ? null : this.getLabelForValue(index);
            },
            maxRotation: 45,
            minRotation: 0,
            color: colors.axisText
          },
          grid: {
            display: false
          }
        }
      }
    }
  });
}
